...
- e70cb3c: `TheoForm` renders again. `0.5.1` shipped with five bare `React.createElement` calls and no `React` import, so any page mounting the component died with `ReferenceError: React is not defined` — and an error boundary took the whole route down with it.
...
- eb08aa3: Um erro de campo devolvido por uma action local volta a aterrar no input.
...
- cce2eb1: `TheoForm` takes its `useAction` from `theokit/client` instead of `@theokit/react`.
