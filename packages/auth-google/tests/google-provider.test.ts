...
describe('google() — accepts a Web Request', () => {
  // TheoKit's route handler hands a Web `Request`; the SDK's AuthProvider interface types this
  // parameter as Node's `IncomingMessage`. Accepting only the Node shape is what made these
  // providers impossible to wire into a TheoKit route (#68).
  it('reads code and state from a Web Request exactly as from an IncomingMessage', async () => {
    fetchSpy
      .mockResolvedValueOnce(jsonResponse(DISCOVERY_DOC))
      .mockResolvedValueOnce(jsonResponse({ access_token: 'ya29.web', expires_in: 3600 }))
      .mockResolvedValueOnce(
        jsonResponse({
          sub: 'sub-from-web-request',
    ...
    const request = new Request(
      `https://myapp.test/api/auth/google/callback?code=auth-code-xyz&state=${TX.state}`,
    )
    const result = await google(OPTS).handleCallback(request, TX)

    expect(result.profile.sub).toBe('sub-from-web-request')
    expect(result.profile.email).toBe('web@example.com')
  })

  it('rejects a Web Request whose state does not match the transaction', async () => {
    fetchSpy.mockResolvedValueOnce(jsonResponse(DISCOVERY_DOC))
    const request = new Request('https://myapp.test/api/auth/google/callback?code=c&state=forged')
    await expect(google(OPTS).handleCallback(request, TX)).rejects.toThrow(/state/i)
  })
})
