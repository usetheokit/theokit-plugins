...
describe('P#11 T4.1 — copilot as RoomMember (multi-user integration)', () => {
  it('alice asks a question and a copilot in the same room responds with full frame chain', async () => {
    ...
    expect((messageBroadcast as { payload: Record<string, unknown> }).payload.text).toBe(
      'Hello Alice, I can help with that.',
    )
    expect((messageBroadcast as { payload: Record<string, unknown> }).payload.role).toBe(
      'assistant',
    )
    expect((messageBroadcast as { payload: Record<string, unknown> }).payload.copilotId).toBe(
      'assistant',
    )

    // Presence snapshot includes both alice + copilot.
    const snapshot = await provider.getPresence('support-room')
    expect(Object.keys(snapshot)).toEqual(expect.arrayContaining(['alice', 'copilot:assistant']))
