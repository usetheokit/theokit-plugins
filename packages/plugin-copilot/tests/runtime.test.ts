...
describe('CopilotRuntime', () => {
  ...
  it('getUsage returns 0 when no charges yet', () => {
    const provider = makeMemoryProvider()
  ...
  it('T2.1: deactivate drains pending queue', async () => {
    ...
    expect(callOrder.indexOf('agent-end')).toBeLessThan(callOrder.indexOf('deactivate-done'))
  })
      ...
      const ctx = errSpy.mock.calls
        ...
        .find(
          (a): a is Record<string, unknown> =>
            typeof a === 'object' && a !== null && 'copilotId' in a,
        )
